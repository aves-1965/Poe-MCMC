...
# Metropolis-Hastings. Version pensada para leerse con facilidad.
#
# IDEA GENERAL:
...
puntaje <- function(f) {
  ...
  n <- length(descifrado)               # cuántas letras tiene el mensaje
...
una_cadena <- function(n_pasos) {
  ...
  mejor_pf <- pf
  historia <- numeric(n_pasos)  #registro del puntaje en cada paso
  ...
  for (i in 1:n_pasos) {
    ...
    if (aceptar) {
      ...
      historia[i] <- pf            # guarda el puntaje actual de esta cadena
    }
  }

  list(f = mejor_f, puntaje = mejor_pf, historia = historia)
}
...
# ---------------------------------------------------------------------------
n_reinicios <- 100      # cuantas cadenas correr
n_pasos     <- 2500     # pasos por cadena

mejor_global_f  <- NULL
mejor_global_historia <- NULL     # junto a las otras variables "mejor_global_*"
mejor_global_pf <- -Inf
...
for (r in 1:n_reinicios) {
  ...
  if (resultado$puntaje > mejor_global_pf) {
    ...
    mejor_global_historia <- resultado$historia
...
plot(cummax(mejor_global_historia), type = "l", col = "steelblue",
     xlab = "paso de la cadena", ylab = "mejor puntaje hasta el paso",
     main = "Cómo la cadena ganadora encontró el descifrado")

#-----------------------------------------------------------------------
# Las coincidencias en general son grandes obstáculos para esa clase 
# de pensadores que han sido educados para no saber nada de la teoría 
# de las probabilidades, esa teoría a la que los objetos más gloriosos 
# de la investigación humana deben las más gloriosas ilustraciones. 
